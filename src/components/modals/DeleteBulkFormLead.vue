<template>
  <div v-if="isDeleteBulkModalOpen" class="qcb-modal" id="delete-selected-leads" @keydown.esc="$emit('close')">
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all qcb-modal-content">
        <div class="px-6 py-5 border-b border-gray-100">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 m-0!">Delete Submissions</h3>
        ...
        <div class="px-6 py-6">
          <div class="text-center">
            <div>
              <p class="text-gray-800 font-medium mb-2! mt-0! text-base!">Are you sure you want to delete this submission?</p>
              <p class="text-sm! text-gray-500 my-0!">This submission settings will be permanently removed from your account.</p>
            </div>
          </div>
        </div>
        <div class="px-6 py-4 bg-gray-50 rounded-b-2xl flex items-center justify-end gap-3">
          ...
          <button type="button" class="px-6 py-2 bg-red-600 text-white rounded-lg font-medium! text-base! hover:bg-red-700 transition-all whitespace-nowrap cursor-pointer disabled:opacity-60" :disabled="isDeleting" @click="$emit('confirm')">
            {{ isDeleting ? 'Deleting...' : 'Delete Submissions' }}
          </button>
        </div>
      </div>
    </div>
