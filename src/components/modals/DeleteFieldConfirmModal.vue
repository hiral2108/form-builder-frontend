<template>
  <div v-if="isDeleteModalOpen" class="qcb-modal" id="delete-field-modal" @keydown.esc="$emit('close')">
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 z-[11111] qcb-modal-overlay" @click="$emit('close')">
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all qcb-modal-content" @click.stop>
        <div class="px-6 py-5 border-b border-gray-100">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 m-0!">Delete Form Field</h3>
        ...
        <div class="px-6 py-6">
          <div class="text-center">
            <p class="text-gray-800 font-medium mb-2! mt-0! text-base!">
              Are you sure you want to delete <span class="font-bold text-teal-600">"{{ fieldLabel }}"</span>?
            </p>
            <p class="text-sm! text-gray-500 my-0!">All collected responses for this field will be lost if you remove it from the builder.</p>
          </div>
        </div>
        <div class="px-6 py-4 bg-gray-50 rounded-b-2xl flex items-center justify-end gap-3">
          ...
          <button type="button" class="px-6 py-2 bg-red-600 text-white rounded-lg font-medium! text-base! hover:bg-red-700 transition-all whitespace-nowrap cursor-pointer" @click="$emit('confirm')">
            Delete Field
          </button>
        </div>
      </div>
