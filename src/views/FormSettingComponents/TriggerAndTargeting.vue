<template>
  <div class="grid grid-cols-12 gap-6 h-full relative items-start">
    ...
    <section class="col-span-12 lg:col-span-7 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-auto">
        <div class="flex items-center gap-3 p-4">
          <div class="w-10 h-10 bg-teal-100 text-teal-700 rounded-lg flex items-center justify-center flex-[0_0_40px]">
            <img v-svg-inline src="@/assets/icons/trigger-targeting/flashlight-line.svg" class="w-5 h-5" alt="trigger settings">
          </div>
          <div>
        ...
        <div class="space-y-5 p-5 border-t border-slate-200 bg-white">
          <!-- Time Delay Trigger -->
          <div class="border border-slate-200 rounded-xl overflow-hidden bg-white">
            <div class="flex items-center justify-between p-4 bg-slate-50/50">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-white border border-slate-200 rounded-lg flex items-center justify-center flex-[0_0_40px]">
                  <img v-svg-inline src="@/assets/icons/trigger-targeting/time_delay.svg" class="text-slate-400 w-5 h-5" alt="Time Delay">
                </div>
                <div>
            ...
            <div class="p-4 border-t border-slate-200 bg-white" v-show="timeDelaySetting.is_time_delay === 1">
              <label class="block text-xs font-semibold text-slate-500 tracking-wider mb-2 flex items-center gap-1" for="delay_duration">
                Delay Duration
                <div class="relative group cursor-pointer leading-none mt-auto">
                  <img v-svg-inline src="@/assets/icons/trigger-targeting/info.svg" class="w-3.5 h-3.5" alt="Info Tooltip">
                  <div class="absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none w-44 text-center z-10 font-normal leading-relaxed">
          ...
          <div class="border border-slate-200 rounded-xl overflow-hidden bg-white">
            <div class="flex items-center justify-between p-4 bg-slate-50/50">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-white border border-slate-200 rounded-lg flex items-center justify-center flex-[0_0_40px]">
                  <img v-svg-inline src="@/assets/icons/trigger-targeting/scroll_based.svg" class="text-slate-400 w-5 h-5" alt="Scroll Based Trigger">
                </div>
                <div>
            ...
            <div class="p-4 border-t border-slate-200 bg-white" v-show="scrollBasedSetting.is_scroll_trigger === 1">
              <label class="block text-xs font-semibold text-slate-500 tracking-wider mb-2 flex items-center gap-1" for="scroll_percentage">
                Scroll Percentage
                <div class="relative group cursor-pointer leading-none mt-auto">
                  <img v-svg-inline src="@/assets/icons/trigger-targeting/info.svg" class="w-3.5 h-3.5" alt="Info Tooltip">
                  <div class="absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none w-48 text-center z-10 font-normal leading-relaxed">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-base font-bold text-slate-700 flex items-center flex-wrap gap-1">
                ...
                <span v-if="userStore.plan_id === 1" class="px-2 py-0.5 bg-gradient-to-r from-red-500 to-orange-500 text-white text-[10px] font-bold rounded-full shadow-sm pointer-events-none">Upgrade to Pro</span>
              </h3>
        ...
        <div class="border-t border-gray-200" v-show="pageRuleSetting.has_page_rule === 1">
          <div class="p-5 relative" :class="userStore.plan_id === 1 ? 'pro-content' : ''">
            <div class="space-y-3">
              <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl" v-for="(rule, key) in pageRuleSetting.rule_setting" :key="rule.id">
                <div class="flex items-center flex-wrap gap-3 w-full">
                  ...
                  <div class="flex-[0_0_155px] relative w-full sm:w-auto">
                  ...
                  <div class="flex-1 w-full sm:w-auto">
                    <InputField
                      ...
                      :classes="validationErrors[`rule_value_${rule.id}`] ? 'border-red-500' : ''"
                    />
                    <p v-if="validationErrors[`rule_value_${rule.id}`]" class="text-xs text-red-500 mt-1">
                      {{ validationErrors[`rule_value_${rule.id}`] }}
                    </p>
                  </div>
                  <div class="flex-[0_0_30px] flex justify-end">
                    <button type="button" @click="removeRule(rule.id)" class="w-10 h-10 bg-red-100 text-red-700 rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center cursor-pointer">
            ...
            <button type="button" @click="addRule" class="w-full flex items-center justify-center gap-2 px-4 py-2 mt-4 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer text-slate-600 hover:border-teal-400 hover:text-teal-600 transition-all font-semibold text-sm bg-white">
              <img v-svg-inline src="@/assets/icons/dashboardpage/add-line.svg" alt="" class="h-5 w-5">
              Add Rule
            </button>
            
            <!-- PRO Upgrade Overlay -->
            <div v-if="userStore.plan_id === 1" class="absolute inset-0 bg-white/70 flex items-center justify-center z-10 rounded-b-xl">
              <router-link to="/plan" class="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white rounded-lg font-semibold text-sm shadow-md">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible">
        ...
        <div class="border-t border-gray-200" v-show="dateTimeSetting.has_date_rule === 1">
          <div class="p-5 relative" :class="userStore.plan_id === 1 ? 'pro-content' : ''">
            <div class="date-time-rule-container space-y-3">
              <div class="grid grid-cols-12 gap-3 items-center">
                <div class="col-span-6 min-w-50">
                  <div class="relative">
                    <label class="block text-sm font-medium text-gray-700 mb-2" for="date_rule_timezone">Timezone</label>
                    <UTCSelectField :options="UTCTimes" v-model="dateTimeSetting.timezone" />
                  </div>
                </div>
              </div>
              <div class="space-y-3 mt-5 date-time-rules">
                <div class="p-4 bg-gray-50 rounded-xl date-time-rule" v-for="(dateRule, key) in dateTimeSetting.rule_setting" :key="dateRule.id">
                  <div class="grid grid-cols-1 min-[600px]:grid-cols-2 min-[900px]:grid-cols-[repeat(4,minmax(0,1fr))_auto] gap-3 items-end w-full min-w-0">
                    <div>
                    ...
                    <div class="flex min-[900px]:justify-end">
                      <button type="button" @click="removeDateRule(dateRule.id)" class="w-full min-[900px]:w-[42px] h-[42px] bg-red-50 text-red-600 rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center cursor-pointer">
                        <img v-svg-inline src="@/assets/icons/form-list/delete.svg" alt="" class="text-lg">
                      </button>
                    </div>
                  </div>
            ...
            <button type="button" @click="addDateRule" class="w-full flex items-center justify-center gap-2 px-4 py-2 mt-4 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer text-slate-600 hover:border-teal-400 hover:text-teal-600 transition-all font-semibold text-sm bg-white">
              <img v-svg-inline src="@/assets/icons/dashboardpage/add-line.svg" alt="" class="h-5 w-5">
              Add Rule
            </button>
        ...
        <div class="border-t border-gray-200" v-show="dayHourSetting.has_day_hour_rule === 1">
          <div class="p-5 relative" :class="userStore.plan_id === 1 ? 'pro-content' : ''">
            <div class="day-hours-rule-container space-y-3">
              ...
              <div class="flex flex-col min-[651px]:flex-row items-stretch min-[651px]:items-center gap-3 p-3 bg-gray-50 rounded-xl min-h-[66px] days-and-hours-schedule" v-for="(dayRule, dayKey) in dayHourSetting.time_schedule" :key="dayKey">
                ...
                <div v-show="dayRule.status === 1" class="days-and-hours-time-section day-schedule-rules space-y-2 flex-1 w-full">
                  <div class="flex flex-col min-[426px]:flex-row items-stretch min-[426px]:items-center gap-2 flex-1 day-schedule-rule" v-for="(timeRule, key) in dayRule.rule_setting" :key="key">
                    ...
                    <span class="text-slate-500 text-sm text-center min-[426px]:text-left font-medium select-none px-1">to</span>
                    <div class="relative flex-1">
                      <el-time-select
                    ...
                    <div v-if="key !== dayRule.rule_setting.length - 1" class="flex-shrink-0 flex min-[426px]:justify-start">
                      <button type="button" @click="removeDayHourRule(dayKey, key)" class="w-9 h-[38px] bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors flex items-center justify-center cursor-pointer">
                        <img v-svg-inline src="@/assets/icons/form-list/delete.svg" alt="" class="text-lg">
                      </button>
                    </div>
                    <div v-else class="flex-shrink-0 flex min-[426px]:justify-start">
                      <button type="button" @click="addDayHourRule(dayKey)" class="bg-teal-100 hover:bg-teal-200 text-teal-700 h-[38px] w-9 rounded-lg cursor-pointer transition-colors flex items-center justify-center">
                        <img v-svg-inline src="@/assets/icons/dashboardpage/add-line.svg" alt="" class="h-5 w-5">
                      </button>
                    </div>
                  </div>
        ...
        <div class="border-t border-gray-200" v-show="countryTargeting.has_country_rule === 1">
          <div class="p-5 relative country-dropdown-box" :class="userStore.plan_id === 1 ? 'pro-content' : ''">
            <div class="country-rule-container space-y-3">
              ...
              <div class="flex flex-wrap gap-2 mb-2 max-h-48 overflow-y-auto" v-show="selectedCountry.length > 0 && !allSelected && selectedCountry.length <= 4">
                <div v-for="code in selectedCountry" :key="code" class="flex items-center gap-2 px-3 py-1 bg-teal-50 text-teal-800 border border-teal-100 rounded-full text-xs font-semibold">
                  <span class="opacity-60">{{ code }}</span>
                  <span>{{ CountryList?.[code] || '' }}</span>
                  <button type="button" @click="removeBadge(code)" class="ml-1 hover:bg-teal-100 rounded-full p-0.5 cursor-pointer">
                    <img v-svg-inline src="@/assets/icons/submission-page/close-line.svg" class="w-3.5 h-3.5" alt="Close">
                  </button>
                </div>
              </div>
              ...
              <div class="relative" ref="dropdownRef">
                <button type="button" class="w-full flex items-center justify-between p-3 bg-white border border-solid! border-slate-300 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer" @click.stop="toggleDropdown">
                  <span class="text-sm font-medium text-slate-700">{{ countryLabel }}</span>
                  <svg width="22" height="22" class="transition-all duration-300" :class="{'rotate-[180deg]': isOpen}" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M8.32036 8.70667L11.6116 5.41334L12.5577 6.34667L8.32036 10.5867L4.08301 6.34667L5.02908 5.41334L8.32036 8.70667Z" fill="#9CA3AF"></path>
                  </svg>
                </button>
                <div :class="[
                  ...
                  <div class="sticky top-0 bg-slate-50 border-b border-slate-200 p-2 flex items-center gap-2">
                    <input type="text" ref="searchInputRef" v-model="search" placeholder="Search country..." class="flex-1 px-3 py-1.5 border border-slate-300 rounded-lg text-sm bg-white outline-none focus:border-teal-500 transition-all">
                    <div class="flex items-center gap-1.5">
                      <button type="button" @click="selectAll" class="px-2 py-1 bg-teal-100 text-teal-700 hover:bg-teal-200 rounded-lg text-xs font-semibold cursor-pointer">Select All</button>
                      <button type="button" @click="clearAll" class="px-2 py-1 bg-teal-100 text-teal-700 hover:bg-teal-200 rounded-lg text-xs font-semibold cursor-pointer">Clear</button>
                    </div>
                  </div>
                  <div class="divide-y divide-slate-100">
                    <button
                      ...
                      <span class="text-[10px] bg-slate-100 text-slate-500 font-bold px-1.5 py-0.5 rounded">{{ code }}</span>
                      <span class="flex-1 text-left text-sm text-slate-700 font-medium">{{ name }}</span>
                      <span v-show="isSelected(code)" class="text-teal-600">
                        <img v-svg-inline src="@/assets/icons/trigger-targeting/checkMark.svg" class="w-4 h-4" alt="Checked">
                      </span>
                    </button>
                  </div>
                </div>
    ...
    <!-- Right Panel (Preview) -->
    <section class="lg:col-span-5 w-full flex flex-col items-center bg-transparent border-none lg:bg-slate-100/50 lg:border lg:border-slate-200/60 rounded-2xl p-0 lg:p-8 min-h-0 lg:min-h-[400px] lg:sticky lg:top-24">
      <PreviewTemplate>
        <DisplayRulePreview />
      </PreviewTemplate>
    </section>
...
const { timeDelaySetting, scrollBasedSetting, pageRuleSetting, dateTimeSetting, dayHourSetting, countryTargeting, validationErrors } = formSetting()
...
const { addRule, removeRule, addDateRule, removeDateRule, disabledStartDateFactory, disabledEndDateFactory, onEndDateChange, onStartDateChange, onDateStartChange, onDateEndChange, addDayHourRule, removeDayHourRule, getDayName, onDayStartChange, onDayEndChange, isOpen, search, dropdownRef, searchInputRef, CountryList, selectedCountry, toggleCountry, removeBadge, allSelected, selectAll, clearAll, toggleDropdown, filteredCountries, countryLabel, isSelected, fetchCountryList } = targetingSetting()
...
<style scoped>
/* Range Slider Custom Teal Styling */
input[type="range"] {
...
input[type="range"]::-webkit-slider-thumb {
  ...
  width: 14px;
  height: 14px;
  border: 1px solid #0d9488; /* Teal ring */
  border-radius: 50%;
  cursor: pointer;
...
/* Days & Hours Schedule Style Overrides */
.day-schedule-rules :deep(.el-input) {
  height: 42px !important;
}

.day-schedule-rules :deep(.el-input__wrapper) {
  border-radius: 12px !important; /* Set border radius to 12px (rounded-xl) */
  height: 42px !important;
...
.day-schedule-rules :deep(.el-input__wrapper:focus-within) {
  box-shadow: 0 0 0 2px #0d9488 inset !important; /* teal focus ring */
}
</style>